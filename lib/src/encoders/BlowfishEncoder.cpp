...
namespace{
    ...
    /*
     * ensure that message size div 2 blocks of uint32_t for feistel net
     */
    std::vector<uint32_t> readMessage(std::istream& in){
        ...
        if(byte != 0){
            alignedMessage[word] = (alignedMessage[word] << 8) | static_cast<uint8_t>(EOT);
            ++byte;
        }
        else{
            alignedMessage.push_back(0x00000000);
...
void BlowfishEncoder::encode(std::istream& in, std::ostream& out) {
    if (!in.good())
    ...
    for(int i = 0; i < message.size(); i += 2){
        uint64_t temp = feistel_net(message[i], message[i + 1], P, SBoxes, false);
